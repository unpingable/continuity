...
| [`MAPSKEW_OBSERVATION_SIDE_V0.md`](MAPSKEW_OBSERVATION_SIDE_V0.md) | V0 specimen implemented (`src/artifact_observer/`) | The light source for MapSkew, not the comparator. A read-only `ArtifactObserver` — its own package, zero continuity coupling — emits one bounded artifact-state claim (subject / claim_kind / claim_value / observed_at / source_ref / can_testify). Continuity remembers; the observer observes; MapSkew (later) compares. No comparison, no skew, no memory, no general scanner. Builds MapSkew's missing input; does not graduate MapSkew. |
